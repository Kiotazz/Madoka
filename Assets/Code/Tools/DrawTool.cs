...
{
    static Material matDraw;

    private static LineRenderer GetLineRenderer(Transform t)
    {
        if (!matDraw) matDraw = Resources.Load<Material>("Effects/Common/Materials/DrawRangeMat");
        LineRenderer lr = t.GetOrAddComponent<LineRenderer>();
        lr.enabled = true;
        lr.startWidth = 0.1f;
        lr.endWidth = 0.1f;
        lr.material = matDraw;
    ...
    public static void DrawSector(Transform t, Vector3 center, float angle, float radius, Color color)
    {
        ...
        float posY = Common.GetGroundHeight(center.x, center.z, center.y + 1000);
        center.y = posY;
    ...
    public static void DrawCircleSolid(Transform t, Vector3 center, float radius)
    {
        ...
        {
            Vector3 pos = Quaternion.Euler(0f, eachAngle * i, 0f) * forward * radius + center;
            pos.y = Common.GetGroundHeight(pos.x, pos.z);
            vertices.Add(pos);
        }
